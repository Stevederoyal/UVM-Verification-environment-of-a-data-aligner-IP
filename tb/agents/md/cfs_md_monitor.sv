...
class cfs_md_monitor#(int unsigned DATA_WIDTH = 32) extends uvm_ext_monitor#(.VIRTUAL_INTF(virtual cfs_md_if#(DATA_WIDTH)), .ITEM_MON(cfs_md_item_mon));
  ...
  protected virtual task collect_transaction();
    ...
    //Detecting start of an item with the help of the delay
    ...
    while(vif.ready !== 1) begin
      ...
      if(agent_config.get_has_checks())begin
        if(item.length >= agent_config.get_stuck_threshold())begin
          `uvm_error("PROTOCOL ERROR", $sformatf("The MD transfer reached the stuck threshold of %0d clock cycles", item.length));
        end
      end
    end

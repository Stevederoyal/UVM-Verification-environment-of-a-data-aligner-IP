...
class cfs_md_sequencer_base_slave extends cfs_md_sequencer_base#(cfs_md_item_drv_slave);
  ...
  virtual function void write(cfs_md_item_mon item);
    //If it is the item at the start of the transaction
    if(item.is_active()) begin
      
      if(pending_items.is_full()) begin
        `uvm_fatal("ALGORITHM ISSUE", $sformatf("FIFO %0s is full (size = %0d) - a possible cause is that there is no sequence started which pulls information from this FIFO", pending_items.get_full_name(), pending_items.size()))
      end
      
      if(pending_items.try_put(item) == 0) begin
        `uvm_fatal("ALGORITHM ISSUE", $sformatf("Failed to push a new item in the FIFO %0s", pending_items.get_full_name()));           
      end
  ...
  virtual function void handle_reset(uvm_phase phase);
    ...
    pending_items.flush(); //Clearing the FIFO
  endfunction

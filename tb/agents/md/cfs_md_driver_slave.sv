...
class cfs_md_driver_slave#(int unsigned DATA_WIDTH = 32) extends cfs_md_driver#(DATA_WIDTH,cfs_md_item_drv_slave);
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
    ...
    if($cast(agent_config, super.agent_config) == 0) begin
      `uvm_fatal("ALGORITHM ISSUE", $sformatf("Could not cast %0s to %0s", super.agent_config.get_full_name(),cfs_md_agent_config_slave#(DATA_WIDTH)::type_id::type_name))
    end
  ...
  protected virtual task drive_transaction(cfs_md_item_drv_slave item);
    ...
    if(vif.valid !== 1) begin
      `uvm_error("ALGORITHM ISSUE", $sformatf("Trying to drive a slave item when there is no item started by the master - item: %0s", item.convert2string()))
    end

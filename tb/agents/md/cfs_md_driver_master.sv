...
class cfs_md_driver_master#(int unsigned DATA_WIDTH = 32) extends cfs_md_driver#(DATA_WIDTH, cfs_md_item_drv_master);
  ...
  protected virtual task drive_transaction(cfs_md_item_drv_master item);
    ...
    if(item.offset + item.data.size() > data_width_in_bytes) begin
      `uvm_fatal("ALGORITHM ISSUE", $sformatf("Trying to drive and item with offset %0d and %0d bytes but the width of the data bus, in bytes, is %0d", item.offset, item.data.size(), data_width_in_bytes))
    end
    
    //Wait for pre drive delay clk cycles
    ...
    //Wait for post drive delay clk cycles

`ifndef CFS_ABP_COVERAGE_SV
  `define CFS_ABP_COVERAGE_SV
...
class cfs_apb_coverage extends uvm_ext_coverage#(.VIRTUAL_INTF(cfs_apb_vif), .ITEM_MON(cfs_apb_item_mon));
  ...
  //at which the bit of the PADDR is 0
  uvm_ext_cover_index_wrapper#(`CFS_APB_MAX_ADDR_WIDTH) wrap_cover_addr_0;
  
  //Wrapper over the coverage group covering the indices of the PADDR signal
  //at which the bit of the PADDR is 1
  ...
  virtual function void write_item(cfs_apb_item_mon item);
    ...
    for(int i = 0; i<`CFS_APB_MAX_DATA_WIDTH; i++)begin
      case(item.dir)
        ...
        default: begin
          `uvm_fatal("ALGORITHM ISSUE", $sformatf("Current version of the code does not support item.dir: %s",item.dir.name()))
        end

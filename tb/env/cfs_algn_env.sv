...
class cfs_algn_env#(int unsigned ALGN_DATA_WIDTH = 32) extends uvm_env implements uvm_ext_reset_handler;
  ...
  virtual function void connect_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(cfs_algn_vif)::get(this,"",vif_name, vif)) begin
      `uvm_fatal("NO_VIF", $sformatf("Could not get from the database the virtual interface name using \"%0s\"", vif_name))
    end
    else begin
    ...
    //Connect the APB sequencer to the address map in order to use
    //the API of the registers to start APB transactions
    ...
    //Pass information from the MD agent to the model
    ...
    if($cast(virtual_sequencer.md_rx_sequencer, md_rx_agent.sequencer)) begin
      `uvm_info("DEBUG",  "Successfully pointing to md_rx_agent.sequencer", UVM_LOW)
    end
    
    if($cast(virtual_sequencer.md_tx_sequencer, md_tx_agent.sequencer)) begin
      `uvm_info("DEBUG",  "Successfully pointing to md_tx_agent.sequencer", UVM_LOW)
    end

...
class cfs_algn_env_config extends uvm_component;
  ...
  //Threshold, in clock cycles, in which to wait for the TX response
  ...
  virtual function void set_algn_data_width(int unsigned value);
    //The minimul legal value for this field is 8
    if(value < 8) begin
      `uvm_fatal("ALGORITHM_ISSUE", $sformatf("The minimum value for algn_data_width is 8 but user trid to set it to %0d", value))
    end
  ...
  //Getter for the virutual interface
  ...
  virtual function void start_of_simulation_phase(uvm_phase phase);
    ...
    if(get_vif() == null) begin
      `uvm_fatal("ALGORITHM ISSUE", "The Aligner virtual interface is not configured at \"Start of simulation\" phase")
    end
    else begin

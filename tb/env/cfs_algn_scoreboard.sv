`ifndef CFS_ALGN_SCOREBOARD
  `define CFS_ALGN_SCOREBOARD
...
class cfs_algn_scoreboard extends uvm_component implements uvm_ext_reset_handler;
  ...
  //Pointer to environment configuration class
  ...
  //Analysis implementation port for receiving TX information from TX MD agent
  ...
  //Expected responses on TX interface
  ...
  //Processes associated with task exp_tx_response_watchdog()
  ...
  //Process assosiated with task rcv_irq()
  ...
  //Function to kill all processes from a queue
    ...
    fork
      begin
        ...
        if(process_exp_rx_response_watchdog.size() == 0) begin
          `uvm_fatal("ALGORITHM_ISSUE", "At the end of the task exp_rx_response_watchdog() the queue of processes process_exp_rx_response_watchdog is empty")
        end
  ...
  //Task for waiting for DUT to output its TX response
    ...
    fork
      begin
        ...
        if(process_exp_tx_item_watchdog.size() == 0) begin
          `uvm_fatal("ALGORITHM_ISSUE", "At the end of the task exp_tx_item_watchdog() the queue of processes process_exp_tx_item_watchdog is empty")
        end
    ...
    fork
      begin
        ...
        if(process_exp_irq_watchdog.size() == 0) begin
          `uvm_fatal("ALGORITHM_ISSUE", "At the end of the task exp_irq_watchdog() the queue of processes process_exp_irq_watchdog is empty")
        end
